...
TA.UI = {
  ...
  layoutKey() {
    ...
    return [this.tab, visB, visT, visC, visU, visJ, gathers, tabs, g.s.victory ? "v" : "", g.s.techs.timeanchor ? "a" : "", g.canVictory() ? "w" : ""].join("|");
  },
  ...
  patchActions() {
    ...
    root.querySelectorAll("[data-craft][data-qty='1']").forEach((btn) => {
      ...
      const ok = TA.canAfford(res, TA.DATA.crafts[id].cost);
      this.setDisabled(btn, !ok);
      const row = btn.closest(".item-row");
      if (row) {
        row.classList.toggle("unaffordable", !ok);
    ...
    root.querySelectorAll("[data-craft][data-qty='max']").forEach((btn) => {
      this.setDisabled(btn, !TA.canAfford(res, TA.DATA.crafts[btn.dataset.craft].cost));
    });
    ...
    root.querySelectorAll("[data-job-count]").forEach((el) => {
      el.textContent = String(g.s.jobs[el.dataset.jobCount] || 0);
    });

    const banner = root.querySelector("[data-banner]");
    if (banner) {
      if (this.tab === "village") {
        banner.textContent = `人口 ${g.s.pop} / ${g.housing()}　闲置 ${idle}　每人消耗食物 ${TA.FOOD_PER_POP}/秒　安定 ${TA.fmt(g.s.happiness, 0)}%`;
  ...
  renderResources() {
    ...
    document.getElementById("pop-summary").textContent = `河民 ${g.s.pop} / ${house} · 闲 ${g.idlePop()}`;
    ...
    for (const id of order) {
      ...
      html += `<div class="res-row"><span class="res-name">${names[id]}</span><span class="res-amt">${TA.fmt(amt)}${cap && cap < 1e8 ? ` <span class="res-cap">/ ${TA.fmt(cap)}</span>` : ""}</span><span class="res-rate ${cls}">${TA.fmtRate(rate)}</span></div>`;
    }
    if (g.s.ember > 0) {
      html += `<div class="res-row"><span class="res-name">余烬</span><span class="res-amt">${TA.fmt(g.s.ember, 0)}</span><span class="res-rate zero">回溯残留</span></div>`;
    }
  ...
  renderTabs() {
    ...
    document.getElementById("tabs").innerHTML = TA.TABS.map((t) => {
      ...
      return `<button type="button" class="tab ${this.tab === t.id ? "active" : ""} ${locked ? "locked" : ""}" data-tab="${t.id}">${t.name}</button>`;
    }).join("");
  },
  ...
  mainHTML() {
    const g = this.game;
    if (g.s.victory) {
      return `<div class="ending"><h2>归乡</h2><p>曲率泡沫在河面上绽开。你看见2300年的地球：土星轨道外的三体舰队，谈判桌上未签完的字，以及仍有人在看天的城市。</p><p>河民的文明被折叠进舰体。智子干扰塔先于你的声音抵达。这一次，人类不是在黑暗里等死。</p><p>胸腔里，时间之锚仍在跳。假如失败——你还可以回来。假如成功——这条河会成为地球的第二条命。</p><p class="muted">回溯 ${g.s.prestige.resets} 次 · 余烬 ${TA.fmt(g.s.ember, 0)} · 本地第 ${g.s.time.year} 年</p></div>`;
    }
    switch (this.tab) {
  ...
  riverHTML() {
    ...
    let html = `<p class="flavor">芯片在胸口跳了一下。它记得火，却不能替你拾柴。先积下原木、纤维和食物，把「用火」解码出来——智库从一开始就能进去。</p><div class="gather-grid">`;
    for (const [id, d] of gathers) {
      html += `<button type="button" class="btn gather" data-gather="${id}"><span class="g-name">${d.gatherName}</span><span class="g-gain">+${TA.fmt(d.gather * bonus)} ${d.name}</span></button>`;
    }
  ...
  nextTechsHTML() {
    ...
    let html = `<h3 class="section-title">芯片残档</h3><p class="flavor">知识在胸腔里，要用河边的材料把它拽出来。完整科技树在「智库」。</p><div class="row-list">`;
    for (const [id, t] of pending) {
      const ok = TA.canAfford(g.amounts(), t.cost);
      html += `<div class="item-row ${ok ? "" : "unaffordable"}"><div><div class="item-title">${t.name}</div><p class="item-desc">${t.desc}<br>解锁：${t.unlocks}</p></div>
  ...
  villageHTML() {
    ...
    let html = `<p class="flavor">他们不是人类。可他们会在火边坐下，会把工具放回原处。低智慧，足够成为文明的土壤。</p>
      <div class="idle-banner" data-banner>人口 ${g.s.pop} / ${house}　闲置 ${g.idlePop()}　每人消耗食物 ${TA.FOOD_PER_POP}/秒　安定 ${TA.fmt(g.s.happiness, 0)}%</div>
      <div class="jobs">`;
    for (const [id, job] of Object.entries(TA.DATA.jobs)) {
      ...
      html += `<div class="job-row"><div><div class="item-title">${job.name}</div><p class="item-desc">${job.desc}<br>${prod}</p></div>
        <div class="job-ctrl">
          <button class="btn btn-tiny" data-job="${id}" data-d="-1">−</button>
          <strong data-job-count="${id}">${n}</strong>
          <button class="btn btn-tiny" data-job="${id}" data-d="1">+</button>
        </div></div>`;
    }
    html += `</div><h3 class="section-title">居所</h3><div class="row-list">${this.buildingRows("river", true)}</div>`;
    return html;
  },

  workshopHTML() {
    const g = this.game;
    let html = `<p class="flavor">工坊里没有智子锁死的物理。只有手、火、以及芯片里不肯死去的工艺。</p><h3 class="section-title">制造</h3><div class="row-list">`;
    for (const [id, c] of Object.entries(TA.DATA.crafts)) {
      if (c.require?.tech && !g.s.techs[c.require.tech]) continue;
      const ok = TA.canAfford(g.s.resources, c.cost);
      const gain = Object.keys(c.gain).length ? Object.entries(c.gain).map(([k, v]) => `${TA.DATA.resources[k].name} +${TA.fmt(v)}`).join(" ") : "加固仓储";
      html += `<div class="item-row ${ok ? "" : "unaffordable"}"><div><div class="item-title">${c.name}</div><p class="item-desc">${c.desc}<br>${gain}</p></div>
    ...
    for (const [id, u] of Object.entries(TA.DATA.upgrades)) {
      ...
      html += `<div class="item-row ${done || ok ? "" : "unaffordable"}"><div><div class="item-title">${u.name}${done ? '<span class="item-count">已完成</span>' : ""}</div><p class="item-desc">${u.desc}</p></div>
  ...
  scienceHTML() {
    const g = this.game;
    let html = `<p class="flavor">人类的全部知识都在你胸口。你必须用这条河的材料，把它们重新变成世界。</p><div class="tech-grid">`;
    ...
    for (const [id, t] of list) {
      ...
      html += `<article class="tech-card ${done ? "done" : ""}"><div class="tech-era">${t.era}</div><h3>${t.name}</h3>
  ...
  spaceHTML() {
    ...
    let html = `<p class="flavor">星空是一条更宽的河。朝地球挥手会被看见；保持安静，才能把箭射出去。</p>
  ...
  anchorHTML() {
    ...
    let html = `<p class="flavor">时间之锚的意义：不是让你永生，而是让你把同一条路走成刀。充能完毕后，你可以回到这个河边——带着余烬。</p>
    ...
    for (const [id, m] of Object.entries(TA.DATA.metaphysics)) {
      ...
      html += `<div class="item-row ${done || ok ? "" : "unaffordable"}"><div><div class="item-title">${m.name}${done ? '<span class="item-count">已铭刻</span>' : ""}</div><p class="item-desc">${m.desc}</p></div>
        <div class="item-cost">${done ? "—" : "余烬 " + m.cost}</div>
  ...
  buildingRows(tab, housingOnly) {
    ...
    for (const [id, b] of Object.entries(TA.DATA.buildings)) {
      if (b.tab !== tab && !(housingOnly && b.housing && b.tab === "river")) continue;
      if (housingOnly && !b.housing) continue;
      if (!housingOnly && tab === "river" && b.housing && this.tab === "river") {
        /* show housing on river too */
      }
      if (housingOnly && this.tab === "village" && !b.housing) continue;
      ...
      html += `<div class="item-row ${ok ? "" : "unaffordable"}"><div><div class="item-title">${b.name}<span class="item-count">${n}</span></div>
